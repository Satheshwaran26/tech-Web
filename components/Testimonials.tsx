    ...
    <section className="bg-gradient-to-b from-[#0a0a0f] via-[#0d0c13] to-[#0a0a0f] py-12 sm:py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-10 sm:mb-16">
          <h2 className="text-gray-300 text-sm font-semibold tracking-wider uppercase mb-3">TESTIMONIALS</h2>
          <h3 className="text-3xl sm:text-4xl md:text-5xl text-gray-300 font-bold">
            Hear from our <span className="bg-gradient-to-r from-pink-500 to-orange-400 text-transparent bg-clip-text">happy</span> customers
          </h3>
        </div>
        <div className="relative">
            ...
            >
              <div className="flex flex-col items-center gap-8">
                ...
                <div className="text-center">
                  <h4 className="text-white font-semibold text-lg sm:text-xl">{testimonials[currentIndex].name}</h4>
          ...
          <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between items-center px-2 sm:px-4 pointer-events-none">
            <button onClick={handlePrevious} className="pointer-events-auto w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm flex items-center justify-center text-white transition-all">
              <FaChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button onClick={handleNext} className="pointer-events-auto w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm flex items-center justify-center text-white transition-all">
        ...
        <div className="flex justify-center gap-2 mt-6 sm:mt-8">
          {testimonials.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentIndex ? "w-6 sm:w-8 bg-purple-500" : "bg-gray-600 hover:bg-gray-500"}`}
            />
          ))}
        </div>
      </div>
